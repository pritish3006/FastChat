...
// import logger from '../tokens/logger';
...
//     if (!this.redis) {
//       logger.info('TokenTracker initialized without Redis (memory tracking disabled)');
//     }
//     if (!this.counter) {
//       logger.info('TokenTracker initialized without TokenCounter (using estimations)');
//     }
//   }
...
//       tokenCount = await this.counter.countTokens(message.content, { model });
//     } else {
//       // Fallback to simple character-based estimation
//       tokenCount = Math.ceil(message.content.length / 4);
//       logger.info('Using character-based token estimation (no TokenCounter available)');
//     }
...
//   ): Promise<TokenUsage> {
//     if (!this.redis) {
//       logger.info('Redis not available, returning in-memory token usage');
//       return {
//         prompt: promptTokens,
...
//     if (!this.redis) {
//       logger.info('Redis not available, returning default token usage');
...
//       prompt: promptTokens,
//       completion: completionTokens,
//       total: promptTotal + completionTokens,
//       windows: {
//         hour: hourlyUsage,
//         day: dailyUsage,
//         month: monthlyUsage
...
//     usage: TokenUsage;
//   }> {
//     if (!this.options.enableRateLimiting || !this.redis) {
